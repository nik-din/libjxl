...
void oned_split_rec(TreeSamples& tree_samples, int32_t l, int32_t r, Tree* tree, size_t tree_pos, size_t depth){
  ...
  for(size_t dim = 0; dim < tree_samples.NumProperties() - tree_samples.NumStaticProps(); dim++){  
    ...
    for(int32_t j = 0; j < tree_samples.NumPredictors(); j++){
      hist[j].resize(max_prop-min_prop+1, std::vector<int32_t>(max_symbols[j], 0));
    }

    for(int32_t j = 0; j < tree_samples.NumPredictors(); j++){
      for(int32_t i = 0; i<max_symbols[j]; i++) hist[j][0][i]=freq[j][0][i];
    }
    for(int32_t h = 0; h < tree_samples.NumPredictors(); h++){
      for(int32_t i = 1; i < max_prop-min_prop+1; i++){
        for(int32_t j = 0; j < max_symbols[h]; j++){
    ...
    std::vector<std::vector<int32_t>> residual_histogramm(tree_samples.NumPredictors());
      ...
      if(i > 0 && exist[i] != -1){
        dp[i] += dp[i-1] + split_compression*FastLog2f(std::abs(tree_samples.UnquantizeProperty(dim+tree_samples.NumStaticProps(), exist[i]))+1) + split_cost + depth*rec_mul;
      }
      opt_split[i] = exist[i];
      ...
      for(int32_t pred_i = 0; pred_i < tree_samples.NumPredictors(); pred_i++){
        ...
        for(int32_t k = 0; k<i; k+=std::max(i/sz,1)){
          ...
          while(x2-x1>1){//check on this
            for(int32_t i1 = 0; i1<max_symbols[pred_i]; i1++){
            ...
            if(x1>0 && exist[x1]!=-1)x11+=dp[x1-1]+split_compression*FastLog2f(std::abs(tree_samples.UnquantizeProperty(dim+tree_samples.NumStaticProps(), exist[x1]))+1)+split_cost + depth*rec_mul; 
            if(x2>0 && exist[x2]!=-1)x21+=dp[x2-1]+split_compression*FastLog2f(std::abs(tree_samples.UnquantizeProperty(dim+tree_samples.NumStaticProps(), exist[x2]))+1)+split_cost + depth*rec_mul;
            if(x11>x21){
              l = x1;
              if(x11 < dp[i]){
                dp[i] = x11; 
          ...
          if(x1 > 0 && exist[x1] != -1){
            new_dp += dp[x1-1] + split_compression*FastLog2f(std::abs(tree_samples.UnquantizeProperty(dim+tree_samples.NumStaticProps(), exist[x1]))+1)  + split_cost + depth*rec_mul;
          }
          if(new_dp < dp[i]){
            dp[i] = new_dp; 
            opt_split[i] = exist[x1];
          ...
          if(x2 > 0 && exist[x2] != -1){
            new_dp += dp[x2-1] + split_compression*FastLog2f(std::abs(tree_samples.UnquantizeProperty(dim+tree_samples.NumStaticProps(), exist[x2]))+1)  + split_cost + depth*rec_mul;
          }
          if(new_dp < dp[i]){
            dp[i] = new_dp; 
            opt_split[i] = exist[x2];
  ...
  for(auto&a:best_cutoffs){
    SplitTreeSamples<false>(tree_samples, (size_t)l1, (size_t)l+best_poss[a], (size_t)r, (size_t)best_property, (uint32_t) a);//off by ones on boundary?
    l1 = l+best_poss[a];
  }
  ...
  while (!q.empty()) {
    ...
    if (info.begin == info.end){
      ...
      else oned_split_rec(tree_samples, l+best_poss[best_cutoffs[info.begin-1]]+1, l+best_poss[best_cutoffs[info.begin]-1]+1, tree, info.pos, depth+1); 
      continue;
    }
