...
void RecursiveSplit(TreeSamples& tree_samples, int32_t l, int32_t r, 
                    Tree* tree, size_t tree_pos, const float split_mul, const float nb_repeats) {
  /**
  ...
  if (best_cutoffs.empty()) {
    if (split_mul * SplitConst::split_mul_mul >= 1) 
      RecursiveSplit(tree_samples, l, r, tree, tree_pos, split_mul * SplitConst::split_mul_mul, nb_repeats);
    return;
  }
      ...
      if (info.begin == 0) {
        if (best_cutoffs[0] > 0) 
          RecursiveSplit(tree_samples, l, l + best_poss[best_cutoffs[info.begin] - 1] + 1, 
                         tree, info.pos, split_mul, nb_repeats); 
        ...
        RecursiveSplit(tree_samples, l + best_poss[best_cutoffs[info.begin - 1]] + 1, 
                      l + best_poss[best_cutoffs[info.begin] - 1] + 1, tree, info.pos, split_mul, nb_repeats); 
      continue;
    }
...
void FindBestCutoff(TreeSamples& tree_samples,
  ...
  RecursiveSplit(tree_samples, 0, tree_samples.NumDistinctSamples(), tree, 0, SplitConst::init_split_mul, nb_repeats);
